<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('course_attachments', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('course_id'); // Changed to unsignedBigInteger
            $table->text('attachment_desc');
            $table->text('attachment_sys_file_name');
            $table->text('attachment_orig_file_name');
            $table->text('attachment_path');
            $table->timestamps();

            // Foreign key constraint with cascading delete
            $table->foreign('course_id')
                ->references('id')
                ->on('courses')
                ->onDelete('cascade'); // Cascade on delete
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('course_attachments');
    }
};
